...
Implemented external API integration using the DummyJSON Quotes API (https://dummyjson.com - keyless, public API) to fetch inspirational quotes for the task management application. Created two endpoints: `/tasks/api/ext/quote/` returns JSON data from the external API with proper error handling and timeout configuration, while `/tasks/quote/` displays formatted quotes in an HTML template with comprehensive error states. Demonstrates requests.get() with timeout handling (5 seconds), and multiple error scenarios including Timeout, ConnectionError, and HTTPError exceptions with appropriate HTTP status codes (504, 503, and pass-through respectively). Implemented a robust fallback mechanism using local mock quotes when the external API is unavailable, ensuring the application remains functional even offline. Extracted and trimmed only relevant fields (quote, author, calculated length) from the API response, discarding unnecessary metadata like id. The implementation showcases both JSON API consumption and template-based display with conditional rendering using {% if %} blocks, providing a practical example of integrating third-party data into a Django application with robust error handling, graceful degradation, and user-friendly fallback messages.

## IP11 Updates

Implemented comprehensive authentication and access control with CSV/JSON export functionality to demonstrate Django's built-in authentication system and data export capabilities. Created a complete authentication flow including custom login page with Illinois blue theme, signup page for external users with auto-login after registration, and logout functionality with proper redirects. Protected all main application views using @login_required for function-based views (task_board, project_detail, tasks_by_priority, task_create_fbv, task_stats, reports_view, export_csv, export_json, task_chart_png) and LoginRequiredMixin for class-based views (TaskStatsView, TaskListView, ProjectDetailView, TaskCreateView, ChartsDashboardView, TaskStatsAPIView). Implemented /reports/ with aggregated totals and breakdowns, plus CSV (`/reports/export/csv/`) and JSON (`/reports/export/json/`) exports. LOGIN_URL, LOGIN_REDIRECT_URL, and LOGOUT_REDIRECT_URL ensure ?next= redirects correctly. Instructor account: username `mohitg2`, password `graingerlibrary`.
